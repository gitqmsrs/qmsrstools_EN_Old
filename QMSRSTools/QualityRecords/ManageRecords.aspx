...
<div id="wrapper" class="modulewrapper">
    ...
    <div id="FilterTooltip" class="tooltip" style="margin-top:10px;">
        <img src="<%=ResolveUrl("~/Images/HTML_Info.gif") %>" alt="Help" height="25" width="25" />
        <p></p>
    </div>
    ...
    <div id="scrollbar" class="gridscroll">
        ...
        <div id="RAGTooltip" class="tooltip" style="margin-top:20px; background-color:transparent;"> 
            <div style="float:left;margin-top:2px; margin-left:2px; width:30%">
                <img id="RED" src="<%=ResolveUrl("~/Images/Red.gif") %>" alt="" style="width:20px; float:left" /><p style="font-size:11px;">Red: Quality Record is Overdue for Review.</p>
            </div>
            <div style="float:left;margin-top:2px; margin-left:2px; width:30%">
                <img id="GREEN" src="<%=ResolveUrl("~/Images/Green.gif") %>" alt="" style="width:20px; float:left" /><p style="font-size:11px;">Green: Quality Record is not due for review.</p>
            </div>
            <div style="float:left;margin-top:2px; margin-left:2px; width:30%">
                <img id="AMBER" src="<%=ResolveUrl("~/Images/Amber.gif") %>" alt="" style="width:20px; float:left" /><p style="font-size:11px;">Amber: Quality Record will be due for review soon.</p>
            </div>
        </div>	
    ...
    <asp:Panel ID="panel1" runat="server" CssClass="modalPanel">
        ...
        <div id="QRTooltip" class="tooltip">
            <img src="<%=ResolveUrl("~/Images/Warning.png") %>" alt="Help" height="25" width="25" />
            <p></p>
	    </div>	
        ...
        <div id="SaveTooltip" class="tooltip">
            <img src="<%=ResolveUrl("/Images/wait-loader.gif") %>" alt="Save" height="25" width="25" />
            <p>Saving...</p>
	    </div>
...
<script type="text/javascript" language="javascript">
    $(function ()
    {
        ...
        $("#fileupload").fileupload(
        {
            ...
            url: '/Upload.ashx',
            add: function (e, data) {
                var acceptFileTypes = $("#<%=RECFTYPCBox.ClientID%>").val().replace(/^.*\./, '');
                var fileExtension = data.originalFiles[0]['name'].replace(/^.*\./, '');

                if (acceptFileTypes != fileExtension) {
                    alert("Please upload a document with file extension '" + $("#<%=RECFTYPCBox.ClientID%>").val() + "'.");
                } else {
                    data.submit();
                }
        ...
        {
            var existingDoc = "." + $("#QRFileDownload").find('p').text().replace(/^.*\./, '');
            var uploadedDoc = "." + $("#filename").val().replace(/^.*\./, '');
            var documentFileType = $("#<%=RECFTYPCBox.ClientID%>").val();

            if (existingDoc != $("#<%=RECFTYPCBox.ClientID%>").val()){
                alert("Please upload a new document with file extension '" + $("#<%=RECFTYPCBox.ClientID%>").val() + "' since the document file type has been changed.");
            }else if( $("#filename").val() != "" && (uploadedDoc != documentFileType)){
                alert("Please upload a new document with file extension '" + $("#<%=RECFTYPCBox.ClientID%>").val() + "'");
            }
            else {
                funcIsPageValid();
            }
                        ...
                        if (result == true) {
                            $("#SaveTooltip").stop(true).hide().fadeIn(500, function () {
                                ...
                                var qualityrecord =
                                {
                                    ...
                                    RecordFileType: $("#<%=RECFTYPCBox.ClientID%>").find('option:selected').text(),
    ...
    function loadGridView(data, empty)
    {
        ...
        $(xmlQR).find('QualityRecord').each(function (index, record)
        {
            ...
            $("td", row).eq(0).html("<img id='icon_" + index + "' src='../RAGHandler.ashx?module=" + $(this).attr('ModuleName') + "&key=" + $(this).attr('RecordID') + "&width=20&height=20&date=" + date.getSeconds() + "' />");
            $("td", row).eq(1).html("<img id='delete_" + index + "' src='<%=ResolveUrl("~/Images/deletenode.png") %>' class='imgButton' />");
            $("td", row).eq(2).html("<img id='edit_" + index + "' src='<%=ResolveUrl("~/Images/edit.png") %>' class='imgButton'/>");
            ...
            var issuedate = new Date($(this).attr("IssueDate"));
            issuedate.setMinutes(issuedate.getMinutes() + issuedate.getTimezoneOffset());

            $("td", row).eq(8).html(issuedate.format("dd/MM/yyyy"));

            var reviewdate = new Date($(this).find("ReviewDate").text());
            reviewdate.setMinutes(reviewdate.getMinutes() + reviewdate.getTimezoneOffset());

            $("td", row).eq(9).html($(this).find("ReviewDate").text() == '' ? '' : reviewdate.format("dd/MM/yyyy"));
            ...
            $(row).find('img').each(function ()
            {
                ...
                else if ($(this).attr('id').search('edit') != -1)
                {
                    $(this).bind('click', function ()
                    {
                        ...
                        bindComboboxAjax2('loadDocumentFileTypes2', "#<%=RECFTYPCBox.ClientID%>", '.' + $(record).attr("RecordFileName").replace(/^.*\./, ''), "#RECFTYP_LD");
                       
                        $("#<%=RECFTYPCBox.ClientID%>").change(function(){
                            if($(this).val() != "0"){
                                $("#QRFileField").parent().show();
                            } else {
                                $("#QRFileField").parent().hide();
                            }
                        })
                        /*bind QR status*/
                        ...
                        $("#<%=ISSUDTTxt.ClientID%>").val(issuedate.format("dd/MM/yyyy"));

                        /*bind review date date*/
                        $("#<%=REVDTTxt.ClientID%>").val($(record).find("ReviewDate").text() == '' ? '' : reviewdate.format("dd/MM/yyyy"));
                        ...
                        else
                        {
                            ...
                            $("#<%=RMKTxt.ClientID%>").val($("#<%=RMKTxt.ClientID%>").html($(record).attr("Remarks")).text());
                        }
                        ...
                        if ($(record).attr("RecordFileName") != '')
                        {
                            ...
                            {
                                $(this).find('img').attr('src', "../ImageHandler.ashx?query=select Icon from DocumentList.DocumentFileType where FileType='" + $(record).attr("RecordFileType") + "'&width=20&height=20");
