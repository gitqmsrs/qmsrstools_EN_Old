...
<script type="text/javascript" language="javascript">
    ...
    function loadGridView(data, empty)
    {
        ...
        $(xmlDOC).find('DocFile').each(function (i, document)
        {
            ...
            {
                $("td", row).eq(0).html("<img id='edit_" + j + "' src='http://www.qmsrs.com/qmsrstools/Images/edit.png' class='imgButton'/>");
                $("td", row).eq(1).html("<img id='icon_" + j + "' src='http://www.qmsrs.com/qmsrstools/Images/download.png' class='imgButton'/>");
                ...
                $("td", row).eq(7).html(new Date($(this).attr("OrginationDate")).format("dd/MM/yyyy"));
                ...
                $(row).find('img').each(function ()
                {
                    ...
                    else if ($(this).attr('id').search('edit') != -1)
                    {
                        $(this).bind('click', function ()
                        {
                            ...
                            $("#<%=ORGNDTTxt.ClientID%>").val(new Date($(DCR).attr("OrginationDate")).format("dd/MM/yyyy"));
