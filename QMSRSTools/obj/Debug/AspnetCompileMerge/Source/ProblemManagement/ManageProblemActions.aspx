...
<div id="wrapper" class="modulewrapper">
    ...
    <div id="ACTTYPContainer" class="filter">
        <div id="ActionTypeFLabel" class="filterlabel">Action Type:</div>
        <div id="ActionTypeFField" class="filterfield">
            <asp:DropDownList ID="ACTTYPFCBox" AutoPostBack="false" runat="server" Width="140px" CssClass="comboboxfilter">
            </asp:DropDownList>
        </div>
    ...
    <div id="RAGTooltip" class="tooltip" style="margin-top:20px; background-color:transparent;"> 
        <div style="float:left;margin-top:2px; margin-left:2px; width:30%">
            <img id="RED" src="http://www.qmsrs.com/qmsrstools/Images/Red.gif" alt="" style="width:20px; float:left" /><p style="font-size:11px;">Red: Problem action has passed target close date.</p>
        </div>
        <div style="float:left;margin-top:2px; margin-left:2px; width:30%">
            <img id="GREEN" src="http://www.qmsrs.com/qmsrstools/Images/Green.gif" alt="" style="width:20px; float:left" /><p style="font-size:11px;">Green: Problem action is on schedule.</p>
        </div>
        <div style="float:left;margin-top:2px; margin-left:2px; width:30%">
            <img id="AMBER" src="http://www.qmsrs.com/qmsrstools/Images/Amber.gif" alt="" style="width:20px; float:left" /><p style="font-size:11px;">Amber: Problem action will be overdue soon.</p>
        </div>
    </div>	
    ...
    <div id="scrollbar" class="gridscroll">
        ...
        <Columns>
            ...
            <asp:BoundField DataField="PRMType" HeaderText="Type" />
    ...
    <asp:Panel ID="panel1" runat="server" CssClass="modalPanel">
        ...
        <div style="float:left; width:100%; height:20px; margin-top:15px;">
            ...
            <div id="ActionDescriptionField" class="fielddiv" style="width:400px;">
                <asp:TextBox ID="ACTDESCTxt" runat="server" CssClass="textbox" Width="390px" Height="90px" TextMode="MultiLine"></asp:TextBox>
            </div>
        ...
        <div style="float:left; width:100%; height:20px; margin-top:15px;">
            ...
            <div id="ActioneeFeedbackField" class="fielddiv" style="width:400px;">
                <asp:TextBox ID="ACTEEFEEDTxt" runat="server" CssClass="textbox" Width="390px" Height="90px" TextMode="MultiLine"></asp:TextBox>
            </div>
    ...
    function loadGridView(data, empty) {
        ...
        $(xmlProblem).find("Problem").each(function (i, problem)
        {
            ...
            $(xmlActions).find('Action').each(function (j, action)
            {
                ...
                $("td", row).eq(0).html("<img id='icon_" + j + "' src='http://www.qmsrs.com/qmsrstools/RAGHandler.ashx?module=" + $(this).attr('ModuleName') + "&key=" + $(this).attr('ActionID') + "&width=20&height=20&date=" + date.getSeconds() + "' />");
                $("td", row).eq(1).html("<img id='delete_" + j + "' src='http://www.qmsrs.com/qmsrstools/Images/deletenode.png' class='imgButton' />");
                $("td", row).eq(2).html("<img id='edit_" + j + "' src='http://www.qmsrs.com/qmsrstools/Images/edit.png' class='imgButton'/>");

                $("td", row).eq(3).html($(this).attr("name"));
                $("td", row).eq(4).html($(problem).attr("Title"));
                $("td", row).eq(5).html($(problem).attr("ProblemType"));
                $("td", row).eq(6).html($(this).attr("ActionType"));
                $("td", row).eq(7).html(new Date($(this).attr("StartDate")).format("dd/MM/yyyy"));
                $("td", row).eq(8).html(new Date($(this).attr("PlannedEndDate")).format("dd/MM/yyyy"));
                ...
                $("td", row).eq(12).html($(problem).attr("ModeString"));
                ...
                $(row).find('img').each(function () {
                    if ($(this).attr('id').search('edit') != -1)
                    {
                        $(this).bind('click', function ()
                        {
                            ...
                            {
                                if ($(problem).find("OriginationDate").text() != '')
                                {
                                    $("#ACTTooltip").find('p').text("Note: The origination date of the problem for this action is on " + new Date($(problem).find("OriginationDate").text()).format("dd/MM/yyyy"));
                                ...
                                {
                                    $("#ACTTooltip").find('p').text("Note: Changes cannot take place on the action, since the origination date of the related problem has not been set");
