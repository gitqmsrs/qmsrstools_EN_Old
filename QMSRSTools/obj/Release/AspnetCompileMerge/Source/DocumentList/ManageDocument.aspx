...
<div id="wrapper" class="modulewrapper">
    ...
    <div id="RAGTooltip" class="tooltip" style="margin-top:20px; background-color:transparent;"> 
        <div style="float:left;margin-top:2px; margin-left:2px; width:30%">
            <img id="RED" src="../Images/Red.gif" alt="" style="width:20px; float:left" /><p style="font-size:11px;">Red: Document is overdue for review.</p>
        </div>
        <div style="float:left;margin-top:2px; margin-left:2px; width:30%">
            <img id="GREEN" src="../Images/Green.gif" alt="" style="width:20px; float:left" /><p style="font-size:11px;">Green: Document is not due for review.</p>
        </div>
        <div style="float:left;margin-top:2px; margin-left:2px; width:30%">
            <img id="AMBER" src="../Images/Amber.gif" alt="" style="width:20px; float:left" /><p style="font-size:11px;">Amber: Document will be due for review soon.</p>
        </div>
    </div>	
...
<div id="header" class="modalHeader">Document Details<span id="close" class="modalclose" title="Close">X</span></div>
    ...
    <div id="DOCWarning" class="tooltip">
        <img src="../Images/Warning.png" alt="Warning" height="25" width="25" />
        <p></p>
	</div>	
    ...
    <div style="float:left; width:100%; height:20px; margin-top:15px;">
        ...
        ControlToValidate="LREVDTTxt" ErrorMessage="Last review date should be greater or equals issue date"  ValidationGroup="General"
    ...
    <div style="float:left; width:100%; height:20px; margin-top:15px;">
        ...
        ControlToValidate="NXTREVDTTxt" ErrorMessage="Next review date should be greater or equals last review date"  ValidationGroup="General"
        Operator="GreaterThanEqual" Type="Date"
        Display="None"></asp:CompareValidator>  

        <asp:CustomValidator id="NXTREVDTTxtF3Val" runat="server" ValidationGroup="General" 
        ControlToValidate = "NXTREVDTTxt" Display="None" ErrorMessage = "Next review date should be in future"
        ClientValidationFunction="compareFuture">
        </asp:CustomValidator>  

        <asp:CustomValidator id="NXTREVDTTxtF4Val" runat="server" ValidationGroup="General" 
...
<script type="text/javascript" language="javascript">
                ...
                success: function (data) {
                    $("#DOCwait").fadeOut(500, function ()
                    {
                        ...
                        $("#FilterTooltip").stop(true).hide().fadeIn(500, function () {
                            ...
                            $(this).find('p').text("List of all  documents filterd according to its mode. Note:Withdrawn, pending, or archived documents cannot be modified");
                        });
                ...
                success: function (data)
                {
                    $("#DOCwait").fadeOut(500, function ()
                    {
                        ...
                        $("#FilterTooltip").stop(true).hide().fadeIn(500, function () {
                            ...
                            $(this).find('p').text("List of all current documents filterd by status. Note:Withdrawn, pending, or archived documents cannot be modified");
                        });
                ...
                success: function (data) {
                    $("#DOCwait").fadeOut(500, function ()
                    {
                        ...
                        $("#FilterTooltip").stop(true).hide().fadeIn(800, function () {
                            ...
                            $(this).find('p').text("List of all current documents filterd by type. Note:Withdrawn, pending, or archived documents cannot be modified");
                        });
            ...
            $.ajax(
            {
                ...
                success: function (data)
                {
                    $("#DOCwait").fadeOut(500, function ()
                    {
                        ...
                        $("#FilterTooltip").stop(true).hide().fadeIn(800, function ()
                        {
                            $(this).find('p').text("List of all current documents filterd by title. Note:Withdrawn, pending, or archived documents cannot be modified");
                        });
            ...
            $.ajax(
            {
                ...
                success: function (data)
                {
                    $("#DOCwait").fadeOut(500, function ()
                    {
                        ...
                        $("#FilterTooltip").stop(true).hide().fadeIn(800, function ()
                        {
                            $(this).find('p').text("List of all current documents. Note:Withdrawn, pending, or archived documents cannot be modified");
                        });
    ...
    function archiveDocument(docID, empty)
    {
        var result = confirm("Are you sure you would like to send this document and all associated change requests to archive?");
        if (result == true)
        {
            $(".modulewrapper").css("cursor", "wait");
            ...
            $.ajax(
            {
                ...
                dataType: "json",
                data: "{'ID':'" + docID + "'}",
                url: getServiceURL().concat('archiveDocument'),
                success: function (data)
                {
                    $(".modulewrapper").css("cursor", "default");
    ...
    function loadGridView(data, empty)
    {
        ...
        $(xml).find("DocFile").each(function (index, value)
        {
            ...
            $("td", row).eq(0).html("<img id='icon_" + index + "' src='../RAGHandler.ashx?module=" + $(this).attr('ModuleName') + "&key=" + $(this).attr('DOCID') + "&width=20&height=20&date=" + date.getSeconds() + "' />");
            $("td", row).eq(1).html("<img id='archive" + index + "' src='../Images/archive.jpg' class='imgButton' />");
            $("td", row).eq(2).html("<img id='delete_" + index + "' src='../Images/deletenode.png' class='imgButton' />");
            $("td", row).eq(3).html("<img id='edit_" + index + "' src='../Images/edit.png' class='imgButton'/>");

            $("td", row).eq(4).html($(this).attr("DOCNo"));
            $("td", row).eq(5).html($(this).attr("DOCFileType"));
            $("td", row).eq(6).html($(this).attr("DOCType"));
            $("td", row).eq(7).html($(this).attr("DOCTitle"));
            $("td", row).eq(8).html($(this).attr("ReviewDuration") + " " + $(this).attr("ReviewPeriod"));
            $("td", row).eq(9).html($(this).find("IssueDate").text() == '' ? '' : new Date($(this).find("IssueDate").text()).format("dd/MM/yyyy"));
            $("td", row).eq(10).html($(this).find("LastReviewDate").text() == '' ? '' : new Date($(this).find("LastReviewDate").text()).format("dd/MM/yyyy"));
            $("td", row).eq(11).html($(this).find("NextReviewDate").text() == '' ? '' : new Date($(this).find("NextReviewDate").text()).format("dd/MM/yyyy"));
            $("td", row).eq(12).html($(this).attr("DOCStatusString"));
            ...
            $(row).find('img').each(function ()
            {
                if ($(this).attr('id').search('edit') != -1)
                {
                    $(this).bind('click', function ()
                    {
                        ...
                        if ($(value).attr('DOCStatusString') == 'Withdrawn' || $(value).attr('DOCStatusString') == 'Pending' || $(value).attr('DOCStatusString') == 'Cancelled')
                        {
                            $("#DOCWarning").find('p').text("Changes cannot take place since the document is " + $(value).attr('DOCStatusString'));
                        ...
                        else if ($(value).attr('ModeString') == 'Archived')
                        {
                            $("#DOCWarning").find('p').text("Changes cannot take place since the document was sent to archive");
                ...
                else if ($(this).attr('id').search('archive') != -1)
                {
                    $(this).bind('click', function ()
                    {
                        ...
                        {
                            alert("Cannot send this document to archive because it is in pending state");
                        }
                        else
                        {
                            archiveDocument($(value).attr('DOCID'), empty);
                        }
                    });
                }
