...
<script type="text/javascript" language="javascript">
        ...
        function loadGridView(data, empty)
        {
            ...
            $(xml).find("Employee").each(function (index, employee) {
                ...
                $("td", row).eq(2).html("<img id='icon_" + index + "' src='http://www.qmsrs.com/qmsrstools/ImageHandler.ashx?query=select ProfileImg from HumanResources.Employee where EmployeeID=" + $(this).attr('EmployeeID') + "&width=70&height=40&date=" + date.getSeconds() + "' />");

                $("td", row).eq(3).html($(this).attr("PersonnelID"));
                $("td", row).eq(4).html($(this).attr("CompleteName"));
                $("td", row).eq(5).html($(this).attr("KnownAs"));
                $("td", row).eq(6).html(new Date($(this).attr("DOB")).format("dd/MM/yyyy"));
                    ...
                    if ($(this).attr('id').search('edit') != -1)
                    {
                        $(this).bind('click', function ()
                        {
                            ...
                            $("#ProfileTooltip").stop(true).hide().fadeIn(800, function ()
                            {
                                ...
                                $('#profileimg').attr("src", "http://www.qmsrs.com/qmsrstools/ImageHandler.ashx?query=select ProfileImg from HumanResources.Employee where EmployeeID=" + $(employee).attr('EmployeeID') + "&width=150&height=150&date=" + date.getSeconds());
                            ...
                            $("#<%=DOBTxt.ClientID%>").val(new Date($(employee).attr("DOB")).format("dd/MM/yyyy"));
                            ...
                            $("#<%=CTRDTTxt.ClientID%>").val(new Date(contract.attr("StartDate")).format("dd/MM/yyyy"));
                            $("#<%=CTREDTTxt.ClientID%>").val(new Date(contract.attr("EndDate")).format("dd/MM/yyyy"));


                            $("#<%=EXTDTTxt.ClientID%>").val(contract.find("ExtendedToDate").text() == '' ? '' : new Date(contract.find("ExtendedToDate").text()).format("dd/MM/yyyy"));
                            $("#<%=TRMDTTxt.ClientID%>").val(contract.find("TerminationDate").text() == '' ? '' : new Date(contract.find("TerminationDate").text()).format("dd/MM/yyyy"));
                            ...
                            $("#<%=DOATxt.ClientID%>").val(new Date(assignement.attr("DOA")).format("dd/MM/yyyy"));
