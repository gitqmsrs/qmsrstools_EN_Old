...
function loadComboboxXML2(xml, classname, field, ID, loader, optionVal) {
    $(ID).empty();
        ...
        if ($(xml).find(classname).size() > 0) {
            ...
            $(xml).find(classname).each(function (index, value) {
                if (optionVal == undefined)
                    ...
                    HTML += "<option value='" + $(this).attr(optionVal) + "'>" + $(this).attr(field) + "</option>";
            });
...
function loadComboboxAjax2(MethodName, ID, loader) {
    $(loader).stop(true).hide().fadeIn(500, function () {
        $.ajax(
        {
            type: "POST",
            ...
            success: function (data) {
                $(loader).fadeOut(500, function () {
                    ...
                    if (data.d.length > 0) {
                        ...
                        var i = 0;
                        $.each(data.d, function (key, value) {
                            HTML += "<option value='" + data.d[i].Extention + "'>" + data.d[i].FileType + "</option>";
                            i++;
                        });
                        $(ID).append(HTML);
                    }
...
function bindComboboxAjax2(MethodName, control, bound, loader) {
    $(loader).stop(true).hide().fadeIn(500, function () {
...
function bindComboboxAjaxKeyValue(MethodName, control, bound, loader) {
    $(loader).stop(true).hide().fadeIn(500, function () {
...
{
    $("#logoheader").attr("src", "/ImageHandler.ashx?query=select CompanyLogo from HumanResources.Company where CompanyID=" + companyID + "&width=324&height=78");
}
...
function bindParamComboboxAjaxKeyValue(MethodName, control, param, bound, loader) {
    var parameter = param.split(":");
...
function loadParamComboboxAjaxKeyValue(MethodName, controls, param, loader) {
    $(loader).stop(true).hide().fadeIn(500, function () {
        var parameter = param.split(":");
        $.ajax(
        {
