              ...
              success: function (data) {
                    $("#TSKwait").fadeOut(500, function () {
                        $("#FilterTooltip").stop(true).hide().fadeIn(800, function () {
                            ...
                            $(this).find('p').text("قائمة بمهام المراجعة الإدارية والتي تمت تصفيتها حسب نوع السجل");
                            if (data) {
    ...
    function loadTaskGridView(data, empty)
    {
        ...
        $(xmlReview).find('Review').each(function (i, review) {
            ...
            $(xmlTasks).find("ReviewTask").each(function (j, task)
            {
                ...
                var plannedclosedate = new Date($(this).attr("PlannedCloseDate"));
                plannedclosedate.setMinutes(plannedclosedate.getMinutes() + plannedclosedate.getTimezoneOffset());

                $("td", row).eq(7).html(plannedclosedate.format("dd/MM/yyyy"));
                ...
                $("td", row).eq(8).html($(this).find("ActualCloseDate").text() == '' ? '' : actualclosedate.format("dd/MM/yyyy"));
                ...
                $(row).find('img').each(function ()
                {
                    if ($(this).attr('id').search('edit') != -1)
                    {
                        $(this).bind('click', function ()
                        {
                            ...
                            $("#<%=CLSDTTxt.ClientID%>").val(plannedclosedate.format("dd/MM/yyyy"));

                            $("#<%=ACTCLSDT.ClientID%>").val($(task).find("ActualCloseDate").text() == '' ? '' : actualclosedate.format("dd/MM/yyyy"));
                            ...
                            else
                            {
                                ...
                                $("#<%=DTLTxt.ClientID%>").val($("#<%=DTLTxt.ClientID%>").html($(task).attr("Description")).text());
                            }
