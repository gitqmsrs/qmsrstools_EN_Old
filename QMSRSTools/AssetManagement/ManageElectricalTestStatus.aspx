<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/Site2.Master" AutoEventWireup="true" Culture = "en-GB" CodeBehind="ManageElectricalTestStatus.aspx.cs" Inherits="QMSRSTools.AssetManagement.ManageElectricalTestStatus" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
<div id="wrapper" class="modulewrapper">
    <div id="ElectricalTestStatus_Header" class="moduleheader">Manage Electrical Test Status</div>

    <div class="toolbox">
        <img id="newstatus" src="<%=GetSitePath() + "/Images/new_file.png" %>" class="imgButton" title="Create new asset electrical test status" alt="" />
        <img id="refreshstatus" src="<%=GetSitePath() + "/Images/refresh.png" %>" class="imgButton" title="Refersh data" alt="" />
    </div>

    <div id="ETSTSloader" class="loader">
    ...
    <asp:Panel ID="panel1" runat="server" CssClass="modalPanel" Style="height: 300px;">
        <div id="header" class="modalHeader">Electrical Test Status Details<span id="close" class="modalclose" title="Close">X</span></div>
        ...
        <div style="float: left; width: 100%; height: 20px; margin-top: 15px;">
            ...
            <asp:RequiredFieldValidator ID="STSNMVal" runat="server" Display="None" ControlToValidate="STSNMTxt" ErrorMessage="Enter the name of the electrical test status" ValidationGroup="General"></asp:RequiredFieldValidator>
                    ...
                    $("#SaveTooltip").stop(true).hide().fadeIn(500, function () {
                        ...
                        if ($("#MODE").val() == 'ADD') {
                            ...
                            $.ajax(
                            {
                                ...
                                dataType: "json",
                                data: "{\'json\':\'" + JSON.stringify(status) + "\'}",
                                url: getServiceURL().concat('updateElectricalTestStatus'),
                                success: function (data)
                                {
                                    $("#SaveTooltip").fadeOut(500, function () {
                                        ActivateSave(true);
...
function removeStatus(ID, empty) {
    var result = confirm("Are you sure you would like to remove the current asset electrical test status record?");
    if (result == true)
    {
       ...
       $.ajax(
       {
           ...
           dataType: "json",
           data: "{'ID':'" + ID + "'}",
           url: getServiceURL().concat("removeElectricalTestStatus"),
           success: function (data)
           {
...
function refresh(empty) {
    $("#ETSTSloader").stop(true).hide().fadeIn(500, function () {
        $(".modulewrapper").css("cursor", "wait");
        ...
        $.ajax(
        {
            ...
            success: function (data)
            {
                $("#ETSTSloader").fadeOut(500, function ()
                {
                    $(".modulewrapper").css("cursor", "default");
