    ...
    function loadGridView(data, empty) {
        ...
        $(xmlProblem).find("Problem").each(function (i, problem)
        {
            ...
            $(xmlActions).find('Action').each(function (j, action)
            {
                ...
                $("td", row).eq(7).html(startdate.format("dd/MM/yyyy"));

                var plannedenddate = new Date($(this).attr("PlannedEndDate"));
                plannedenddate.setMinutes(plannedenddate.getMinutes() + plannedenddate.getTimezoneOffset());

                $("td", row).eq(8).html(plannedenddate.format("dd/MM/yyyy"));
                ...
                $("td", row).eq(9).html($(this).find("ActualCloseDate").text() == '' ? '' : actualclosedate.format("dd/MM/yyyy"));
                ...
                $(row).find('img').each(function () {
                    if ($(this).attr('id').search('edit') != -1)
                    {
                        $(this).bind('click', function ()
                        {
                            ...
                            $("#<%=STRTDTTxt.ClientID%>").val(startdate.format("dd/MM/yyyy"));

                            /*bind action target close date*/
                            $("#<%=ENDDTTxt.ClientID%>").val(plannedenddate.format("dd/MM/yyyy"));

                            /*bind action actial close date*/
                            $("#<%=ACTCLSDTTxt.ClientID%>").val($(action).find("ActualCloseDate").text() == '' ? '' : actualclosedate.format("dd/MM/yyyy"));
                            ...
                            else
                            {
                                ...
                                $("#<%=ACTDESCTxt.ClientID%>").val($("#<%=ACTDESCTxt.ClientID%>").html($(action).attr("Description")).text());
                            ...
                            else
                            {
                                ...
                                $("#<%=ACTEEFEEDTxt.ClientID%>").val($("#<%=ACTEEFEEDTxt.ClientID%>").html($(action).attr("ActioneeFeedback")).text());
