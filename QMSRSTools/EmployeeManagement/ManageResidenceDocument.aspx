...
<div id="wrapper" class="modulewrapper">
    ...
    <div class="toolbox">
        ...
        <img id="new" src="/Images/new_file.png" class="imgButton" title="Add New Residence Document" alt="" />  
    ...
    <script type="text/javascript" language="javascript">
        ...
        function loadResidenceGridView(data, empty) {
            ...
            $(xml).find("ResidenceDocument").each(function (index, residence)
            {
                ...
                var validfrom = new Date($(this).attr("ValidFrom"));
                validfrom.setMinutes(validfrom.getMinutes() + validfrom.getTimezoneOffset());

                $("td", row).eq(3).html(validfrom.format("dd/MM/yyyy"));

                var validto = new Date($(this).attr("ValidTo"));
                validto.setMinutes(validto.getMinutes() + validto.getTimezoneOffset());

                $("td", row).eq(4).html(validto.format("dd/MM/yyyy"));
                ...
                $(row).find('img').each(function () {
                    ...
                    else if ($(this).attr('id').search('edit') != -1)
                    {
                        $(this).bind('click', function ()
                        {
                            ...
                            $("#<%=VLDFRMTxt.ClientID%>").val(validfrom.format("dd/MM/yyyy"));

                            $("#<%=VLDTOTxt.ClientID%>").val(validto.format("dd/MM/yyyy"));
