...
<script language="javascript" type="text/javascript">
        ...
        function loadGridView(data, empty) 
        {
            ...
            $(xml).find("Position").each(function (index, value) {
                ...
                var opendate = new Date($(this).attr("OpenDate"));
                opendate.setMinutes(opendate.getMinutes() + opendate.getTimezoneOffset());

                $("td", row).eq(4).html(opendate.format("dd/MM/yyyy"));

                var closedate = new Date($(this).attr("CloseDate"));
                closedate.setMinutes(closedate.getMinutes() + closedate.getTimezoneOffset());

                $("td", row).eq(5).html(closedate.format("dd/MM/yyyy"));
                ...
                $(row).find('img').each(function () {
                    if ($(this).attr('id').search('delete') != -1) {
                        ...
                        $(this).bind('click', function () 
                        {
                            ...
                            $("#<%=ODateTxt.ClientID%>").val(opendate.format("dd/MM/yyyy"));
                            $("#<%=CLDateTxt.ClientID%>").val(closedate.format("dd/MM/yyyy"));
