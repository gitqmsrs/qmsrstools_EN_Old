...
<div id="wrapper" class="modulewrapper">
    ...
    <div id="ActionScrollbar" class="gridscroll">
        ...
        <Columns>
            ...
            <asp:BoundField DataField="ActionID" HeaderText="Action No." />
        ...
        $(xmlAudits).find('AuditRecord').each(function (i, audit)
        {
            ...
            $(xmlFinding).find("AuditFinding").each(function (j, finding)
            {
                ...
                $(xmlActions).find("AuditActions").each(function (k, action)
                {
                    ...
                    var completeddate = new Date($(this).find("CompleteDate").text());
                    ...
                    $("td", row).eq(3).html($(audit).attr("AuditNo"));
                    $("td", row).eq(4).html($(finding).attr("FindingID"));
                    $("td", row).eq(5).html($(this).attr("ActionID"));
                    $("td", row).eq(6).html($(audit).attr("AuditType"));
                    $("td", row).eq(7).html(planneddate.format("dd/MM/yyyy"));
                    $("td", row).eq(8).html($(finding).attr("Finding"));
                    $("td", row).eq(9).html($(this).attr("ActionType"));
                    $("td", row).eq(10).html(targetclosingdate.format("dd/MM/yyyy"));
                    $("td", row).eq(11).html($(this).find("DelayedDate").text() == '' ? '' : delayeddate.format("dd/MM/yyyy"));
                    $("td", row).eq(12).html($(this).find("CompleteDate").text() == '' ? '' : completeddate.format("dd/MM/yyyy"));
                    $("td", row).eq(13).html($(this).attr("Actionee"));
                    $("td", row).eq(14).html($(audit).attr("AuditStatusString"));
                    $("td", row).eq(15).html($(this).attr("IsClosed") == 'true' ? 'Closed' : 'Open');
                    ...
                    $(row).find('img').each(function ()
                    {
                        if ($(this).attr('id').search('edit') != -1)
                        {
                            $(this).bind('click', function ()
                            {
                                ...
                                $("#<%=CMPLTDTTxt.ClientID%>").val($(action).find("CompleteDate").text() == '' ? '' : completeddate.format("dd/MM/yyyy"));
                                ...
                                if ($(action).attr("Details") == '') {
                                    ...
                                    $("#<%=ACTDTLTxt.ClientID%>").val($(action).attr("Details"));
                                ...
                                if ($(action).attr("FollowUpComments") == '') {
                                    ...
                                    $("#<%=FLLCOMTxt.ClientID%>").val($(action).attr("FollowUpComments"));
                                }
