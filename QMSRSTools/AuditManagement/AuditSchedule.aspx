...
<div id="wrapper" class="modulewrapper">
    ...
    <div id="RAGTooltip" class="tooltip" style="margin-top:20px; background-color:transparent;"> 
        <div style="float:left;margin-top:2px; margin-left:2px; width:30%">
            <img id="RED" src="<%=GetSitePath() + "/Images/Red.gif" %>" alt="" style="width:20px; float:left" /><p style="font-size:11px;">Red: The audit is overdue.</p>
        </div>
        <div style="float:left;margin-top:2px; margin-left:2px; width:30%">
            <img id="GREEN" src="<%=GetSitePath() + "/Images/Green.gif" %>" alt="" style="width:20px; float:left" /><p style="font-size:11px;">Green: The audit is on schedule.</p>
        </div>
        <div style="float:left;margin-top:2px; margin-left:2px; width:30%">
            <img id="AMBER" src="<%=GetSitePath() + "/Images/Amber.gif" %>" alt="" style="width:20px; float:left" /><p style="font-size:11px;">Amber: The audit has been delayed.</p>
        </div>
    </div>	
    ...
    <asp:Panel ID="panel1" runat="server" CssClass="modalPanel">
        ...
        <div id="DetailsTB" class="tabcontent" style="display:none; height:450px;">
            ...
            <div style="float:left; width:100%; height:20px; margin-top:10px;">
                ...
                <div id="AUDTIDField" class="fielddiv" style="width:250px">
                    <asp:TextBox ID="AUDTIDTxt" runat="server" CssClass="readonly" Width="90px" ReadOnly="true"></asp:TextBox>
                </div>
            </div>
            ...
            <div style="float:left; width:100%; height:20px; margin-top:15px;">
                ...
                <asp:CompareValidator ID="CompareValidator1" runat="server" ControlToCompare="AUDTDTTxt"  ValidationGroup="Details"
                ControlToValidate="ACTAUDTDTTxt" ErrorMessage="Actual audit date should be greater than or equals the planned audit date"
             ...
             <div style="float:left; width:100%; height:20px; margin-top:15px;">
                ...
                <%--<asp:CustomValidator id="ACTCLSDTF0Val" runat="server" ValidationGroup="Details" 
                ControlToValidate = "ACTCLSDTTxt" Display="None" ErrorMessage = "Actual close date shouldn't be in future"
                ClientValidationFunction="comparePast">
                </asp:CustomValidator>--%>

                <asp:CompareValidator ID="ACTCLSDTF1Val" runat="server" ControlToCompare="ACTAUDTDTTxt"  ValidationGroup="Details"
                ControlToValidate="ACTCLSDTTxt" ErrorMessage="Actual close date should be greater than or equals the actual audit date"
                Operator="GreaterThanEqual" Type="Date"
                Display="None"></asp:CompareValidator> 

                <asp:CustomValidator id="ACTCLSDTF2Val" runat="server" ValidationGroup="Details" 
        ...
        <div id="ORGUnitsTB" class="tabcontent" style="display:none;height:450px;">
            ...
            <div class="toolbox">
                <img id="newORG" src="<%=GetSitePath() + "/Images/new_file.png" %>" class="imgButton" title="Add New Organization Unit" alt=""/>
            </div>
        ...
        <div id="AuditorsTB" class="tabcontent" style="display:none;height:450px;">
            <span id="lblAuditorMessage" class="validator"></span>

            <img id="newAuditor" src="<%=GetSitePath() + "/Images/new_file.png" %>" class="imgButton" title="Add new Auditor" alt=""/>
...
<script type="text/javascript" language="javascript">
    ...
    $(function ()
    {
        ...
        $("#<%=ACTCLSDTTxt.ClientID%>").datepicker(
       {
        ...
        $("#save").bind('click', function ()
        {
            ...
            var auditors = getAuditorsJSON();
            var removeAll = true;
            var removeAllauditors = true;
            var hasError = false;
            var memberDuplicates = false;
            var auditorDuplicates = false;

            var org = [];
            for (x = 0; x < units.length; x++) {
                if (units[x].Status != 4) {
                    org.push(units[x].ORGID);
                    removeAll = false;
                }
            }
            org = org.sort();

            for (y = 0; y < org.length; y++) {
                if (org[y + 1] == org[y]) {
                    memberDuplicates = true;
                }
            }
            ...
            var auditorsName = [];
            for (x = 0; x < auditors.length; x++) {
                if (auditors[x].Status != 4) {
                    auditorsName.push(auditors[x].NameFormat);
                    removeAllauditors = false;
                }
            }

            auditorsName = auditorsName.sort();
            for (y = 0; y < auditorsName.length; y++) {
                if (auditorsName[y + 1] == auditorsName[y]) {
                    auditorDuplicates = true;
                }
            }
            ...
            if (removeAll) {
                $("#lblUnitMessage").html("<ul><li>Auditee Unit is required.</li></ul>");
                $("#lblUnitMessage").show();
                navigate('ORGUnits');
                hasError = true;
            } else if (memberDuplicates) {
                $("#lblUnitMessage").html("<ul><li>Duplicate ID exists.</li></ul>");
                $("#lblUnitMessage").show();
                navigate('ORGUnits');
                hasError = true;
            } else if (auditorDuplicates) {
                $("#lblAuditorMessage").html("<ul><li>Duplicate name exists.</li></ul>");
                $("#lblAuditorMessage").show();
                navigate('Auditors');
                hasError = true;
            }
            else if (auditors == null || auditors == "" || (auditors.length == 1 && auditors[0].Status == 4) || (removeAllauditors)) {
                $("#lblAuditorMessage").html("<ul><li>Auditor is required.</li></ul>");
                $("#lblAuditorMessage").show();
                navigate('Auditors');
                hasError = true;
            }
                ...
                if (isDetailsValid)
                {
                    ...
                    if (isAdditionalValid)
                    {
                        ...
                        if (result == true) {
                            ...
                            //    navigate('ORGUnits');
                            //}
                            //else if (auditors == null || auditors == "" || auditors[0].Status == 4) {
                            //    $("#lblAuditorMessage").html("<ul><li>Auditor is required.</li></ul>");
                            //    $("#lblAuditorMessage").show();
                            //    navigate('Auditors');
                            //}
                            //else {
                                ...
                                $("#SaveTooltip").stop(true).hide().fadeIn(500, function () {
                                    ...
                                    var planneddate = new Date(AUDTDTParts[2], (AUDTDTParts[1] - 1), AUDTDTParts[0]);
                                    //var date = new Date();
                                    //planneddate.setHours(date.getHours());
                                    //planneddate.setMinutes(date.getMinutes());
    ...
    function removeUnit(name)
    {
        ...
        $.ajax(
        {
            ...
            success: function (data)
            {
                ...
                var index = orgUnits.indexOf(ORGUNTJSON.ORGID);
                if (index > -1) {
                    orgUnits.splice(index, 1);
                }
                //alert(orgUnits.length);
            },
            error: function (xhr, status, error)
            {
                $(".modulewrapper").css("cursor", "default");
    ...
    function addUnit(name)
    {
        ...
        $.ajax(
        {
            ...
            success: function (data)
            {
                ...
                orgUnits.push(ORGUNTJSON.ORGID);
                //alert(orgUnits.length);
            },
            error: function (xhr, status, error)
            {
    ...
    function bindAuditRecord(data,matrix)
    {
        ...
        $(xml).find('AuditRecord').each(function (index, audit)
        {
            ...
            //alert($(this).find("ActualAuditDate").text());
            //alert(actualauditdate);
            ...
            //alert($(this).attr('PlannedAuditDate'));
            //alert(planneddate);

            //var planneddate = moment($(this).attr('PlannedAuditDate'));
            //alert($(this).attr('PlannedAuditDate'));
            //alert(planneddate);
            ...
            sb.append("<img id='delete_" + index + "' src='<%=ResolveUrl("~/Images/deletenode.png") %>' class='imgButton' style='float:left;' title='Remove audit record'/>");
            sb.append("<img id='edit_" + index + "' src='<%=ResolveUrl("~/Images/edit.png") %>' class='imgButton' style='float:left;' title='Edit audit Record'/>");
            sb.append("<img id='report_" + index + "' src='<%=ResolveUrl("~/Images/report.png") %>' class='imgButton' style='float:left;' title='View report' />");
            sb.append("<img id='RAG_" + index + "' src='/RAGHandler.ashx?module=" + $(this).attr('ModuleName') + "&key=" + $(this).attr('AuditID') + "&width=20&height=20&date=" + date.getSeconds() + "' class='imgButton' style='float:left;' title='RAG Status'/>");
            ...
            sb.append("<div id='PlannedAuditDate_" + index + "' class='tdl' style='width:87px'>" + planneddate.format("dd/MM/yyyy") + "</div>");
            sb.append("<div id='ActualAuditDate_" + index + "' class='tdl' style='width:87px'>" + ($(this).find("ActualAuditDate").text() == '' ? '' : actualauditdate.format("dd/MM/yyyy")) + "</div>");
            ...
            $("#report_" + index).bind('click', function ()
            {
                ...
                var URL = "AuditReport.aspx?AuditID=" + $(audit).attr('AuditID');
                window.open(URL);
            });
            ...
            $("#edit_" + index).bind('click', function ()
            {
                ...
                $("#<%=AUDTDTTxt.ClientID%>").val(planneddate.format("dd/MM/yyyy"));

                /* bind actual audit date*/
                $("#<%=ACTAUDTDTTxt.ClientID%>").val($(audit).find("ActualAuditDate").text() == '' ? '' : actualauditdate.format("dd/MM/yyyy"));

                /* bind actual close date*/
                $("#<%=ACTCLSDTTxt.ClientID%>").val($(audit).find("ActualCloseDate").text() == '' ? '' : actualclosedate.format("dd/MM/yyyy"));
                ...
                if ($(audit).attr('Scope') != '' && $(audit).attr('Scope') != null) {
                    ...
                    $("#<%=SCOPTxt.ClientID%>").val($("#<%=SCOPTxt.ClientID%>").html($(audit).attr('Scope')).text());
                }
                else {
                ...
                if ($(audit).attr('Summery') != '' && $(audit).attr('Summery') != null) {
                    ...
                    $("#<%=SMMRYTxt.ClientID%>").val($("#<%=SMMRYTxt.ClientID%>").html($(audit).attr('Summery')).text());
                }
                else {
                ...
                if ($(audit).attr('Comments') != '' && $(audit).attr('Comments') != null) {
                    ...
                    $("#<%=NOTETxt.ClientID%>").val($("#<%=NOTETxt.ClientID%>").html($(audit).attr('Comments')).text());
                }
                else {
                ...
                loadRecipients($(audit).attr('XMLAuditRecipients'));
